...
export default function TextField({value}: {value: string}) {
	...
	return (
		<div className='pt-5 pb-4.5 	px-8 flex flex-row gap-4 items-center justify-between bg-grey-800 my-4 sm:mt-8 sm:my-6'>
			<input
				type='text'
			...
			<div className='relative'>
				{copied ? (
					<span className='text-preset-4 sm:text-preset-3 uppercase text-green-200 absolute right-7.5 top-1/2 -translate-y-1/2 bg-grey-800 p-2'>
						Copied
					</span>
				) : null}
				<button
					...
					<svg
						...
						<path
							...
							className='fill-green-200 group-hover:fill-white transition-colors duration-200'
						/>
					</svg>
				</button>
			</div>
